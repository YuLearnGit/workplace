        ...
        {          
            ...
            try
            {
                ...
                {                   
                    ...
                    string[] log_prefix = log.Split(']');
                    //Console.WriteLine("{0}",log_prefix[1]);
                    string mon = null;
                    ...
                    if (log_array[1] == "")
                    {
                        string[] prefix_pre1 = log_prefix[1].Split('&');
                        handle_result = prefix_pre1[0];
                        PROTO = prefix_pre1[1];
                        string handle_reason0 = prefix_pre1[2].Split(' ')[0];
                    ...
                    /*按月份建表并存入数据库*/
                    string sqlCreate = "CREATE TABLE if not EXISTS `" + tab_name + "` (`time` datetime DEFAULT NULL,`host_name` varchar(50) DEFAULT NULL,`fw_MAC` varchar(20) DEFAULT NULL,`data_in` varchar(10) DEFAULT NULL,`PHYSIN` varchar(10) DEFAULT NULL,`data_out` varchar(10) DEFAULT NULL,`PHYSOUT` varchar(10) DEFAULT NULL,`DST_MAC` varchar(20) DEFAULT NULL,`SRC_MAC` varchar(20) DEFAULT NULL,`SRC` varchar(20) DEFAULT NULL,`DST` varchar(20) DEFAULT NULL,`LEN` varchar(10) DEFAULT NULL,`TOS` varchar(10) DEFAULT NULL,`PREC` varchar(10) DEFAULT NULL,`TTL` varchar(10) DEFAULT NULL,`ID` varchar(10) DEFAULT NULL,`PROTO` varchar(10) DEFAULT NULL,`SPT` varchar(10) DEFAULT NULL,`DPT` varchar(10) DEFAULT NULL,`handle_result` varchar(30) DEFAULT NULL,`handle_reason` varchar(255) DEFAULT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8;insert into `" + tab_name + "`(time,host_name,fw_MAC,data_in,PHYSIN,data_out,PHYSOUT,DST_MAC,SRC_MAC,SRC,DST,LEN,TOS,PREC,TTL,ID,PROTO,SPT,DPT,handle_result,handle_reason) values('" + time + "','" + host_name + "', '" + fw_MAC + "','" + data_in + "','" + PHYSIN + "','" + data_out + "','" + PHYSOUT + "','" + DST_MAC + "','" + SRC_MAC + "','" + SRC + "','" + DST + "','" + LEN + "','" + TOS + "','" + PREC + "','" + TTL + "','" + ID + "','" + PROTO + "','" + SPT + "','" + DPT + "','" + handle_result + "','" + handle_reason + "')";
                    MySqlCommand cmd_create = new MySqlCommand(sqlCreate, con);                  
                   cmd_create.ExecuteNonQuery();
                    string displayLog =time+" "+host_name+" "+fw_MAC+" "+data_in+" "+PHYSIN+" "+data_out+" "+PHYSOUT+" "+DST_MAC+" "+
                        SRC_MAC+" "+SRC+" "+DST+" "+LEN+" "+TOS+" "+PREC+" "+TTL+" "+ID+" "+PROTO+" "+SPT+" "+DPT+" "+handle_result+" "+handle_reason;
